...
public class Questions {
	...
	public String sandwichFilling(String sandwich) {
		...
		if (firstBread == -1)
			return "";
		String postFirstBread = sandwich.substring(firstBread + 5);
		int secondBread = postFirstBread.indexOf("bread");
		if (secondBread == -1)
			return "";
		String betweenBread = postFirstBread.substring(0, secondBread);
		String[] reversedBetweenBread = betweenBread.split("");
		String output = "";
		for (int i = reversedBetweenBread.length; i > 0; i--) {
			output = output + reversedBetweenBread[i-1];
		}
		return output;
	}
	...
	public boolean evenlySpaced(int a, int b, int c) {
		...
		if (toOrder[1] - toOrder[0] == toOrder[2] - toOrder[1])
			return true;
		return false;
	}
	...
	public String nMid(String input, int n) {
		...
		String leftInput = input.substring(0, (input.length()-1)/2 - (n-1)/2);
		String rightInput = input.substring((input.length()+1)/2 + (n-1)/2);
	...
	public int superBlock(String input) {
		...
		for (int i = 1; i < inputChar.length; i++) {
			if (inputChar[i].equals(inputChar[i-1]))
				currentCharCount++;
			else
				currentCharCount = 0;
			if (currentCharCount + 1 > largestCount)
				largestCount = currentCharCount + 1;
		}
	...
	public int largest(String input) {
		...
		for (int i = 0; i < inputArray.length; i++) {
			int currentSum = 0;
			currentInt = Integer.parseInt(inputArray[i]);
			while (currentInt > 0) {
				currentSum = currentSum + (currentInt % 10);
				currentInt /= 10;
			}
